-- truetrace_schema_azuresql.sql (Azure SQL Database)
-- Same schema as truetrace_schema.sql, adapted for Azure SQL Database:
--   - No CREATE DATABASE / DROP DATABASE / USE / ALTER DATABASE SET SINGLE_USER: Azure SQL
--     Database is provisioned as its own resource (Azure Portal, az sql db create, or your
--     existing setup) and a script connects directly to ONE database -- you can't switch
--     databases mid-script the way on-prem SQL Server allows.
--   - Connect directly to the `truetrace` database (not `master`) before running this, e.g. in
--     SSMS/Azure Data Studio: Server = <your-server>.database.windows.net, Database = truetrace.
--   - Tables are dropped individually (IF EXISTS) instead of wiping the whole database, since on
--     Azure SQL you likely don't want to drop the database resource itself.
--
-- Before running this:
--   1. The `truetrace` database must already exist on your Azure SQL server (create it via the
--      Portal, or: az sql db create -g <resource-group> -s <server-name> -n truetrace --service-objective S0)
--   2. Your client IP must be allowed through the server's firewall (Portal: your SQL server ->
--      Networking -> Firewall rules -> Add your client IPv4 address; or `az sql server firewall-rule create`)
