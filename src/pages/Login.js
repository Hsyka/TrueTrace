...
export default function Login() {
  ...
  // Test credentials (keep for demo)
  ...
  useEffect(() => {
    ...
    window.google.accounts.id.renderButton(
      ...
      { shape: "rectangular", theme: "outline", size: "large", text: "signin_with", logo_alignment: "left" }
    );
  ...
  const handleSubmit = async (e) => {
    ...
    // Allow hardcoded test login
    if (email === TEST_EMAIL && password === TEST_PASSWORD) {
      const testUser = { id: "test-user-123", email: TEST_EMAIL, name: "Test User" };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-6">
      <div className="w-full max-w-md bg-white p-8 rounded-xl shadow">
        <h1 className="text-3xl font-bold text-gray-900 mb-6 text-center">Login to TrueTrace</h1>
        ...
        {error && <p className="text-red-500 text-sm">{error}</p>}
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full ${loading ? "bg-blue-300" : "bg-blue-500 hover:bg-blue-600"} text-white font-semibold rounded-lg py-2 transition`}
          >
        ...
        <p className="mt-4 text-center text-sm text-gray-600">
          ...
          <Link to="/signup" className="text-blue-500 hover:text-blue-600 font-medium">Create one</Link>
        </p>
      </div>
    </div>
  );
